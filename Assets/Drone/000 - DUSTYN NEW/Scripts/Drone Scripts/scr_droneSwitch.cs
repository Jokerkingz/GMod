...
public class scr_droneSwitch : MonoBehaviour {
	...
	void Switching()
	{
       ...
       /* float step = droneStats.rotateTowardsSpeed * Time.deltaTime;
        droneSource.lookAtSystem.LookAt(switchPoints[curSwitchPoint].position);
        transform.rotation = Quaternion.RotateTowards(transform.rotation, droneSource.lookAtSystem.rotation, step);*/
